...
export default function Stock() {
  ...
  const onUpdate = async (id, data) => {
    ...
    try {
      const response = await axios.patch(
        `https://inventory-app-backend-zma5.onrender.com/api/inventory/${id}`,
        data
      );
  ...
  const onDelete = async (id) => {
    const deleteData = inventories.filter((inventory) => inventory._id !== id);
    setInventories(deleteData);
    setOriginalInventories(deleteData);
    try {
